...
static int drawitem(struct item *item, int x, int y, int w) {
    ...
    if (iscomment == 2) {
        if (item->text[2] == ' ') {
            ...
            dest[6] = '\0';
            drw_text(drw, x, y, temppadding, lineheight, temppadding / 2.6, dest + 3,
                    0, item == sel);
            iscomment = 6;
            drw_setscheme(drw,
                    sel == item ? scheme[SchemeHover] : scheme[SchemeNorm]);
        }
    }
    ...
    return drw_text(
            ...
            output + iscomment, 0, (iscomment == 3 || item == sel));
}
...
static void drawmenu(void) {
    ...
    if (prompt && *prompt) {
        drw_setscheme(drw, scheme[SchemeSel]);
        if (lines < 2) {
            x = drw_text(drw, x, 0, promptw, bh * (lines + 1), lrpad / 2, prompt, 0,
                    1);
        } else {
    ...
    if (lines > 0) {
        ...
        for (item = curr; item != next; item = item->right)
            x = drawitem(
                    item, x, 0,
                    MIN(TEXTW(item->stext), mw - x - TEXTW(">") - TEXTW(numbers)));
...
static void grabpointer(void) {
    XGrabPointer(dpy, root, False, MOUSEMASK, GrabModeAsync, GrabModeAsync, None,
            XCreateFontCursor(dpy, XC_fleur), CurrentTime);
}
...
void animatesel(struct item *item) {
    ...
    while (time < framecount) {
        ...
        if (sely + lineheight < mh - 10)
            drw_rect(drw, 0, sely + (lineheight - 4), mw,
                    (easeOutQuint(((double)time / framecount)) *
                     (mh - (lineheight - 4) - sely)),
                    1, 1, 0);
        // top animation
        drw_rect(drw, 0,
                sely + 4 -
                (easeOutQuint(((double)time / framecount)) * (sely + 4)),
                mw, (easeOutQuint(((double)time / framecount)) * sely), 1, 1, 0);
        drw_map(drw, win, 0, 0, mw, mh);
        time++;
        usleep(19000);
    }
...
void animaterect(int x1, int y1, int w1, int h1, int x2, int y2, int w2,
        int h2) {
    if (!animated || !framecount)
        return;
    int time;
    ...
    while (time < framecount) {
        ...
        drw_rect(drw, x1 + (x2 - x1) * timefactor, y1 + (y2 - y1) * timefactor,
                w1 + (w2 - w1) * timefactor, h1 + (h2 - h1) * timefactor, 1, 1, 0);
        drw_map(drw, win, 0, 0, mw, mh);
        time++;
        usleep(19000);
    }
        ...
        if (alttab) {
            if (sel) {
                if (sel == items) {
                    ...
                    for (lastitem = items; lastitem && lastitem->right;
...
insert:
        ...
        case XK_Home:
            ...
            break;
        case XK_Left:
            if ((ev->state & ShiftMask || ev->state & Mod4Mask) &&
                    (leftcmd || rightcmd)) {
                char *tmpcmd;
                if (leftcmd)
                    tmpcmd = leftcmd;
...
static void setselection(XEvent *e) {
    struct item *item;
...
static void setup(void) {
    ...
    lines = MAX(lines, 0);
    mh = (lines + 1) * bh;
    promptw = commented             ? bh * 15
        : (prompt && *prompt) ? TEXTW(prompt) - lrpad / 4
        : 0;
    ...
    swa.event_mask = ExposureMask | KeyPressMask | KeyReleaseMask |
        VisibilityChangeMask | ButtonPressMask | PointerMotionMask;
    ;
    ...
    if ((xrm = XResourceManagerString(drw->dpy))) {
        ...
        for (int i = 0; i < SchemeLast; ++i) {
            for (int j = 0; j < ColLast; ++j) {
                char xresprop[100] = "";
                snprintf(xresprop, sizeof(xresprop), "%s.%s.%s", xresname,
                        xresscheme[i], xrescolortype[j]);
